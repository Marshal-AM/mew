 ...
 for (int i = 0; i < numAlign; i++) {
  for (int j = 0; j < numAlign; j++) {
   ...
   for (int dy = -1; dy <= 1; dy++) {
    for (int dx = -1; dx <= 1; dx++) setModuleBounded(qrcode, alignPatPos[i] + dx, alignPatPos[j] + dy, dx == 0 && dy == 0);
   }
  }
 }
 if (version >= 7) {
...
static long getPenaltyScore(const uint8_t qrcode[]) {
 ...
 for (int y = 0; y < qrsize - 1; y++) {
  for (int x = 0; x < qrsize - 1; x++) {
   ...
   if (color == getModuleBounded(qrcode, x + 1, y) && color == getModuleBounded(qrcode, x, y + 1) && color == getModuleBounded(qrcode, x + 1, y + 1))
    result += PENALTY_N2;
  }
 }
 int dark = 0;
 for (int y = 0; y < qrsize; y++) {
  for (int x = 0; x < qrsize; x++) if (getModuleBounded(qrcode, x, y)) dark++;
 }
...
testable void setModuleBounded(uint8_t qrcode[], int x, int y, bool isDark) {
 ...
 if (isDark) qrcode[byteIndex] |= 1 << bitIndex;
...
testable int calcSegmentBitLength(enum qrcodegen_Mode mode, size_t numChars) {
 ...
 else if (mode == qrcodegen_Mode_ECI && numChars == 0) result = 24;
...
testable int getTotalBits(const struct qrcodegen_Segment segs[], size_t len, int version) {
 ...
 for (size_t i = 0; i < len; i++) {
  ...
  if (numChars >= (1L << ccbits)) return LENGTH_OVERFLOW;
