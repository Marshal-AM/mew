...
#define OLED_SCL_PIN  9   // "SCK" on your module label = same signal as SCL
#define OLED_I2C_ADDR 0x3C

Adafruit_SSD1306 display(OLED_WIDTH, OLED_HEIGHT, &Wire, -1);

const char* TEST_TEXT = "hello";   // short, fixed — same idea as the Hackster demo
...
void setup() {
  ...
  Wire.setClock(100000); // slow and tolerant
  ...
  // Solid fill, then checkerboard. If THESE look wrong, it's not QR-related at all.
  ...
  // --- Step 2: fixed-version hardcoded QR, same approach as the working example ---
  display.clearDisplay();

  const uint8_t version = 3; // 29x29 modules, matches the Hackster demo
